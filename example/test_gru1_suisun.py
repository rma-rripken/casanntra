...
class GRUBuilder1(ModelBuilder):
    ...
    def build_model(self, input_layers, input_data):
        ...
        x = layers.GRU(units=16, return_sequences=False, activation='sigmoid', name='gru_2')(x)
        x = layers.Flatten()(x)
    ...
    def fit_model(self, ann, fit_input, fit_output, test_input, test_output, nepochs=80):
        history = ann.fit(
            fit_input,
            fit_output,
            epochs=nepochs,
    ...
    def plot_metrics_for_current_fold(self, r2_scores, mse_scores, mae_scores, fold_number):
        ...
        for metric_name, scores in metrics_dict.items():
            ...
            plt.tight_layout()
            plot_filename = f"output/{metric_name.replace(' ', '_').lower()}_fold_{fold_number}.png"
...
def test_xvalid_gru():
    ...
    df_in, df_out = builder.xvalid_time_folds(df, target_fold_len='180d', split_in_out=True)
    ...
    save_histories_to_json(
        builder.histories,
        "output/base_model_histories_wmetrics.json"
    )
    ...
    save_metrics_to_json(
        builder.metrics,
        "output/base_model_metrics_wmetrics.json"
    )
