...
class TransferLearningGRUBuilder1(GRUBuilder1):
    ...
    def fit_model(
        self, ann, fit_input, fit_output, test_input, test_output, nepochs=30
    ):
        history = ann.fit(
            fit_input,
            fit_output,
...
def test_transfer_learning_gru1():
    ...
    for col in output_names:
        if col == "x2":
            df_out_base[col] = df_out_base[col] / 100.0
        elif col in ["mal", "cse", "bdl", "nsl2"]:
            df_out_base[col] = df_out_base[col] / 10000.0
        elif col in ["emm2", "jer", "tms"]:
            df_out_base[col] = df_out_base[col] / 2000.0
        else:
            df_out_base[col] = df_out_base[col] / 1000.0

    builder_base = GRUBuilder1(
        input_names=input_names, output_names=output_names, ndays=80
    )
    ...
    plot_training_history(
        ...
        "Transfer Learning Base Model Training History",
        "output/transfer_base_model_history_wmetrics.png",
    )
    ...
    for col in output_names:
        if col == "x2":
            df_out_suisun[col] = df_out_suisun[col] / 100.0
        elif col in ["mal", "cse", "bdl", "nsl2"]:
            df_out_suisun[col] = df_out_suisun[col] / 10000.0
        elif col in ["emm2", "jer", "tms"]:
            df_out_suisun[col] = df_out_suisun[col] / 2000.0
        else:
            df_out_suisun[col] = df_out_suisun[col] / 1000.0
    ...
    save_histories_to_json(
        builder_transfer.histories, "output/transfer_learning_histories_wmetrics.json"
    )
    ...
    save_metrics_to_json(
        builder_transfer.metrics, "output/transfer_learning_metrics_wmetrics.json"
    )
