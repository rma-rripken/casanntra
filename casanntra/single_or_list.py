...
def single_or_list(*arg_names):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            target_args = {name: bound_args.arguments[name] for name in arg_names if name in bound_args.arguments}

            # Determine if any argument is a list
            is_list = {name: isinstance(value, list) for name, value in target_args.items()}
            ...
            list_lengths = {name: len(value) for name, value in target_args.items() if isinstance(value, list)}
            
            if len(set(list_lengths.values())) > 1:
                raise ValueError(f"Arguments {', '.join(list_lengths.keys())} must have the same length when they are lists.")
