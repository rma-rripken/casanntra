...
def single_model_fit(builder, df_in, fit_in, fit_out, test_in, test_out,
    ...
    return pred, history_serialized
...
def bulk_fit(builder, df_in, df_out, out_prefix, fit_in, fit_out, test_in, test_out,
    ...
    outputs_trim = [df.loc[inputs_lagged.index, builder.output_list()] for df in df_out] if isinstance(df_out, list) else df_out.loc[inputs_lagged.index, builder.output_list()]
    ...
    if num_outputs > 1:
      for i in range(num_outputs):
         fit_out[i].to_csv(f"{out_prefix}_xvalid_{i}.csv",
                           float_format="%.3f",
                           date_format="%Y-%m-%dT%H:%M",
                           header=True, index=True)
    else: 
         fit_out.to_csv(f"{out_prefix}_xvalid.csv",
                           float_format="%.3f",
                           date_format="%Y-%m-%dT%H:%M",
                           header=True, index=True)        
...
def xvalid_fit_multi(df_in, df_out, builder, init_train_rate, init_epochs, main_train_rate, main_epochs,
    ...
    for future in concurrent.futures.as_completed(futures):
        try:
            ...
            test_pred, history_encoded = future.result()
    ...
    if isinstance(outputs_xvalid,list):
      ...
      for i in range(num_outputs):
         outputs_xvalid[i].loc[:,full_col_list].to_csv(f"{out_prefix}_xvalid_{i}.csv", float_format="%.3f",
                                    date_format="%Y-%m-%dT%H:%M", header=True, index=True)
    else:
        print("Single Structure")
        outputs_xvalid.loc[:,full_col_list].to_csv(f"{out_prefix}_xvalid_.csv", float_format="%.3f",
                                    date_format="%Y-%m-%dT%H:%M", header=True, index=True)
    print("Done writing\n\n")
...
def allocate_receiving_df(df_out,column_list):
    ...
    outputs_xvalid = pd.DataFrame(columns=output_cols).reindex(df_out.index)
    
    # ✅ Preserve datetime, case, and fold info
    outputs_xvalid.loc[:,["datetime", "case", "fold"]] = df_out.loc[:,["datetime", "case", "fold"]]
