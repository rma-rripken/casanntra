...
def append_tidal_pca_cols(df):
    ...
    data_normalized["sf_tidal_filter"] -= data_normalized.groupby("case")["sf_tidal_filter"].transform("mean")
    data_normalized["sf_tidal_filter"] /= data_normalized.sf_tidal_filter.std()
    data_normalized["sf_tidal_energy"] -=  data_normalized.sf_tidal_energy.mean()  # Note global for energy, group for filter
    data_normalized["sf_tidal_energy"] /= data_normalized.sf_tidal_energy.std()
